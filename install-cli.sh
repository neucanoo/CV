...
main() {
  ...
  # Bootstrap Node before doing anything else in this installer. Keep this
  # message explicit so logs prove that this exact install-cli.sh was loaded.
  NODE_VERSION="26.1.0"
  NODE_VERSION_REQUESTED=1
  printf '%s\n' '[install-cli.sh] Loaded: installing Node.js 26.1.0 before all other setup steps.' >&2
  select_node_version_for_platform "$(os_detect)" "$(arch_detect)"
  PATH="$(node_dir)/bin:${PREFIX}/bin:${PATH}"
  export PATH
  if [[ "$NODE_ONLY" -eq 1 ]] && is_musl_linux; then
